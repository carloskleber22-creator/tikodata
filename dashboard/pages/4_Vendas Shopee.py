...
from app.shopee_client import ShopeeAPIError
from app.models import ShopeeAccount
from app.services import shopee_sales
from scripts.seed_demo_data import seed_shopee_demo_data
...
st.set_page_config(page_title="Vendas Shopee", page_icon="🛍️", layout="wide")
init_db()
inject_base_css()
require_demo_login()

st.title("🛍️ Vendas Shopee")
st.caption("Receita e unidades vendidas da sua própria loja na Shopee (requer conexão OAuth).")
st.info(
    "Candidatura como parceiro de software (ISV) enviada em 2026-08-16, em avaliação pela "
    "Shopee — ver README. Sem `partner_id`/`partner_key` reais ainda, então a conexão OAuth "
    "abaixo não funciona de verdade até a aprovação sair. Use dados de exemplo enquanto isso."
)
...
if not accounts:
    st.warning("Nenhuma loja Shopee conectada ainda.")
...
account_map = {a.id: f"{a.shop_name} (#{a.id})" for a in accounts}
account_id = col_a.selectbox("Loja", options=list(account_map.keys()), format_func=lambda i: account_map[i])
with col_b:
    period_options = {"Ontem": 1, "7d": 7, "30d": 30, "90d": 90, "180d": 180}
    period_label = st.pills("Período", options=list(period_options.keys()), default="30d", key="shopee_period")
    days = period_options.get(period_label, 30)

with SessionLocal() as db:
    account = db.query(ShopeeAccount).filter(ShopeeAccount.id == account_id).one()
    is_demo = account.shop_id == "000000000"

if is_demo:
    st.caption("🧪 Conta de demonstração — dados de exemplo gerados localmente, não vêm da Shopee.")
elif st.button("Sincronizar pedidos", type="primary"):
    with st.spinner("Sincronizando pedidos com a Shopee..."):
        with SessionLocal() as db:
            account = db.query(ShopeeAccount).filter(ShopeeAccount.id == account_id).one()
            try:
...
with SessionLocal() as db:
    comparison = shopee_sales.get_period_comparison(db, account_id, days=days)
